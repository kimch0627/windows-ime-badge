...
배지 모습을 다듬은 변경입니다. **클래식을 포함한 모든 테마에서 배지 모습이 조금 달라집니다**. 색은 그대로이고, 벚꽃·캔디의 영문 기본색만 바뀝니다.
설정 파일 형식은 그대로입니다.

### 변경
- **테두리**: 흰/검 반투명 선 대신 배지색을 같은 색조로 어둡게 한 선. 어두운 배지에 흰 테두리가 둘러져 스티커처럼 보이던 것이 없어지고,
  밝은 배경에서도 윤곽이 또렷하다. 불투명도를 낮춰도 테두리는 남아 배지 위치가 보인다.
- **광택**: 모든 테마에 안쪽 위 가장자리의 밝은 림(Windows 11 Fluent 풍). 새 테마의 위쪽 그라데이션은 절반으로 줄였다.
- **글자 정렬**: 글꼴의 줄 높이가 아니라 실제 글자 모양(잉크) 기준으로 배지 가운데에 둔다. "한"·"a"·"A"·"꺆" 이 고르게 가운데에 오고,
  한 글자 영문 배지("a", "A")는 정확한 원이 된다. 배지 폭도 글자 모양에 맞춰 1~2px 좁아질 수 있다. 트레이 아이콘도 같은 방식.
- **영문 글꼴**: 영문·기호("a", "A", "?")를 Segoe UI Bold 로(한글은 맑은 고딕 Bold 그대로). 트레이 아이콘의 "A" 도 같다. Segoe UI 가 없으면 맑은 고딕.
- **불투명도를 낮췄을 때 글자 가독성**: 글자 둘레에 배지색을 옅고 얇게 깔아(헤일로) 뒤의 문서가 비쳐도 글자가 읽힌다.
  예: 클래식 한글 배지의 흰 글자는 불투명도 30% 흰 문서 위에서 대비가 1.6:1 까지 떨어졌다. 불투명도 100% 에서는 달라지지 않는다.
- **벚꽃·캔디의 영문 기본색**을 진하게(벚꽃 `#B6A4F0` → `#8E7CE0`, 캔디 `#4DAEF0` → `#1C74C9`). 한글 기본색과 밝기가 거의 같아
  (휘도 대비 1.08·1.12) 흑백이나 색약으로는 두 배지가 구별되지 않았다. 이제 모든 테마에서 1.5 이상을 테스트로 확인한다.

### 추가
- 개발용 `ImeBadge.exe --render-sheet <경로>`: 모든 테마·모양·캐릭터, 불투명도, 배율, 트레이 아이콘, 한/영 색의 흑백 구분을 한 장의 PNG 로 그린다.
  CI 가 Windows 러너에서 그려 롤링 사전 릴리스에 `badge-sheet.png` 로 붙인다(렌더러가 Windows 전용이라 Linux 테스트로는 모습을 볼 수 없다).
  한/영 전환 애니메이션의 프레임도 실제 화면처럼(같은 caret 기준) 나란히 그린다.
...
- 한/영이 바뀔 때 배지가 살짝 커졌다 돌아오는 애니메이션(0.26초) 동안 글자가 배지 안에서 1px 남짓 오르내리고
  획 굵기가 깜빡이던 문제([#47](https://github.com/kimch0627/windows-ime-badge/issues/47)). 프레임마다 배율을 바꿔 새로 그려서
  글꼴 힌팅·정수 픽셀 맞춤·그림자 여백이 서로 다른 순간에 끊겼다. 이제 기본 크기로 한 번 그린 배지를 확대해 올린다.
