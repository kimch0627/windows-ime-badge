...
| 구역 | 항목 | 설명 |
|---|---|---|
| 모양 | 배지 모양 | 실제로 그린 타일에서 고름: 사각 `[한]` / 둥근 `(한)`(기본) / 점 `●` / 밑줄 `▬` / 점, 바뀔 때 1.5초 글자 |
...
| | 한글/영문 배지 색 | 원형 색 견본(Windows 강조색 11개). 마지막 견본은 사용자 지정(색 대화상자). 글자색(흰/검)은 고른 색의 밝기에 맞춰 자동 |
...
3. **caret 위치** (두 경로 + 마지막 대안)
   ...
   - 이미지 커서 추적(opt-in, `Ime/ImageCaret.cs`): 설정에서 "화면을 분석해 커서를 따라가기"를 켜면, 모서리로 가기 전에 대상 창을
     ...
     배지 움직임을 커서로 오인하는 되먹임을 막습니다. 커서가 깜빡이지 않는 설정이면 타이핑으로 움직일 때만 찾습니다.
     `--debug` 로그에 `caret:img(...)`, 못 찾으면 `img:nodiff`(변화 없음)·`img:reject(WxH)`(커서 모양 아님)가 남습니다.
