...
| 설정... | 아래 설정 창 |
| 모양 / 위치 / 크기 / 투명도 | 자주 쓰는 항목을 메뉴에서 바로 |
| 로그인 시 자동 시작 | 체크하면 Windows 에 로그인할 때 같이 실행됩니다 |
| 업데이트 확인 | 새 정식 버전이 있으면 다운로드 페이지를 안내합니다 |
| 정보... | 버전, 설정·로그 폴더 열기 |
...
| 구역 | 항목 | 설명 |
|---|---|---|
| 모양 | 배지 모양 | 사각 `[한]` / 둥근 `(한)`(기본) / 점 `●` / 밑줄 `▬` / 점 + 바뀔 때만 1.5초 글자 |
| | 위치 | 커서 오른쪽 위(기본) / 아래. 위쪽이 다음 줄 글자를 덜 가림 |
| | 크기 | 50~300 %. 모니터 DPI 배율에 추가로 곱해짐 |
| | 불투명도 | 30~100 %. 낮출수록 뒤 글자가 비쳐 보임 |
| | 한글/영문 배지 색 | 색 고르기 대화상자 |
