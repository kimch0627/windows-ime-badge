...
/// 자동 업그레이드로 내려받은 파일은 %LOCALAPPDATA%\ImeBadge\update (임시 보관).
/// </summary>
public sealed class AppPaths
{
    public string SettingsDir { get; }
    public string LogDir { get; }
    /// <summary>자동 업그레이드로 내려받은 설치 파일을 잠시 두는 곳. 적용한 뒤(또는 다음 실행 때) 지운다.</summary>
    ...
    public AppPaths(string settingsDir, string logDir, string? legacySettingsFile = null, string? updateDir = null)
    {
        SettingsDir = settingsDir;
        LogDir = logDir;
        LegacySettingsFile = legacySettingsFile;
        UpdateDir = updateDir ?? Path.Combine(logDir, "update");
    }
    ...
    public static AppPaths Default()
    {
        ...
        return new AppPaths(
            ...
            Path.Combine(local, AppInfo.ProductName, "update"));
    }
}
